...
Можно расположить в три колонки

```
.columns.three-columns
  .column
    h4 Ищет по всем каналам
    p Если человек выбирает услугу индивидуального подбора, кандидатов для него ищут по всем каналам, а не только по базе агентства. Тут работают разные средства: поиск в интернете, рекомендации знакомых и социальные сети.
  .column
    h4 Выступает за честность
    p В социальных сетях наше агентство выступает за честность. Если находим подходящего кандидата, так и пишем: «Мы из брачного агентства, у нас есть мужчина или женщина в поиске серьезных отношений, и, кажется, вы могли бы познакомиться.
  .column
    h4 Смотрит на фотографии
    p При поиске кандидатов в социальных сетях обращаем внимание на фотографии, привлекают фото с улыбкой, на которых видно, что человек открытый. А стильные фотографии без улыбок, например черно-белые с загадочным взглядом, привлекают меньше.
```

![pics/columns-three-columns.png](pics/columns-three-columns.png)
...
Линейки можно убрать классом `.no-lines`
...
.table.no-lines
  .tr
    .td
      h4.figure 399 млн ₽
      p разработчики и поставщики оборудования, специалисты по IOS-продакшн
    .td
      h4.figure 22,2 млн ₽
      p исследователи и экспериментаторы
  .tr
    .td
      h4.figure 33,6 млн ₽
      p компании по защите информации
    .td
      h4.figure 16 млн ₽
      p поставщики лицензий на ПО
```

![pics/table-no-lines.png](pics/table-no-lines.png)
...
Чтобы считать засчитывать в результат ошибки, вариантам с ошибкой даем `value="1"` (как в тестах «сколько ошибок вы допустите»). Чтобы красить эти ответы в красный, используем на тесте класс `.count-mistakes`. 

```jade
.quiz.true-false.count-mistakes
```
...
noindex
  .business-slider
    ...
    .blue-plate.input-values
      ...
      .slider-caption(data-start-value="250000" data-end-value="250000" data-step="100000")
...
В последнем значении `data-start-value` и `data-end-value` должны совпадать.
...
Прогрессивная шкала распределения цифр. К инпуту добавить класс `.pow`. Используется степенная шкала для распределения и цифры расставятся на ползунке равномернее. Подходит для случаев, когда есть и небольшие и крупные числа, например тысячи и миллионы.

```jade
input.pow(type="range" min="0" max="12000000" value="0")
```

Для дробных чисел с остатком после запятой надо добавить атрибут `data-digits`. Он регулирует количество точек после запятой. По умолчанию 0, то есть числа будут округляться до целых.
...
```
  .business-slider
    ...
    input(type="range" min="1.2" max="8.9" value="0" data-digits="1")
```
